                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (mergeQueue.empty()) 
    {
        TreeNode *x = singleQueue.front();
        singleQueue.pop();
        return x;
    } 
    else if (singleQueue.empty()) 
    {
        TreeNode *y = mergeQueue.front();
        mergeQueue.pop();
        return y;
    }
    TreeNode *first = singleQueue.front(), *second = mergeQueue.front();
    if (first -> freq < second -> freq || first -> freq == second -> freq) 
    {
        singleQueue.pop();
        return first;
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
    for (const auto& i : frequencies) singleQueue.push(new TreeNode(i));
    while (singleQueue.size() + mergeQueue.size() >= 2)
    {
        ...
        TreeNode *cur = new TreeNode(left -> freq.getFrequency() + right -> freq.getFrequency());
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (!bfile.getNextBit())
    {
        cur = new TreeNode(Frequency(0));
        cur -> left = readTree(bfile);
        cur -> right = readTree(bfile);
    }
    else 
    {
        cur = new TreeNode(Frequency(bfile.getNextByte(), 0));
    }
    return cur;
}
