...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode* build(int l, int r, int curDim);
    size_t partition(size_t l, size_t r, int curDim);
    void copy(KDTreeNode* x, KDTreeNode* y) {
      if (y -> left != nullptr) {
        x -> left = new KDTree<Dim>::KDTreeNode();
        x -> left -> point = y -> left -> point;
        copy(x -> left, y -> left);
      }
      if (y -> right != nullptr) {
        x -> right = new KDTree<Dim>::KDTreeNode();
        x -> right -> point = y -> right -> point;
        copy(x -> right, y -> right);
      }
    }
    void deleteTree(KDTreeNode* cur) {
      if (cur == nullptr) return;
      deleteTree(cur -> left);
      deleteTree(cur -> right);
      delete cur;
    }
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode* cur, int curDim) const;
};
